...
# Функция запроса уникальной оценки к Gemini под конкретный ник
async def evaluate_username_with_gemini(username: str) -> str:
    prompt = (
        f"Проанализируй телеграм-никнейм '@{username}' состоящий из букв. "
        f"На основе его звучания, сочетания гласных/согласных и запоминаемости вынеси уникальную оценку. "
        f"Не пиши одинаковые шаблонные цифры, оцени объективно именно этот набор букв ({username}). "
        f"Выдай ответ строго в таком формате и больше ничего лишнего:\n\n"
        f"├── Читаемость - [уникальная цифра от 1 до 10]/10\n"
        f"├── Примерная цена - [$минимальная-$максимальная]\n"
        f"├── Ликвидность - [уникальная цифра от 1 до 10]/10\n"
        f"└── ⚡️ Свободен"
    )
...
async def process_auto_find(callback: types.CallbackQuery):
    ...
    await callback.message.edit_text(
        f"⏳ Ищу свободный ник ({length} букв, только буквы). Перебираю варианты...",
        parse_mode="Markdown"
    )

    found_username = None

    # Бесконечный цикл поиска до нахождения свободного ника
